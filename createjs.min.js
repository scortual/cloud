...
a.types.length;b<c;b++)this._typeCallbacks[a.types[b]]=a.callback;if(null!=a.extensions){b=0;for(c=a.extensions.length;b<c;b++)this._extensionCallbacks[a.extensions[b]]=a.callback}}};a.setMaxConnections=function(a){this._maxConnections=a;!this._paused&&0<this._loadQueue.length&&this._loadNext()};a.loadFile=function(a,b,c){
null==a?this._sendError({text:"PRELOAD_NO_FILE"}):(this._addItem(a,c),!1!==b?this.setPaused(!1):this.setPaused(!0))};a.loadManifest=function(a,b,c){var f=null;if(a instanceof Array){if(0==
...
//=======================================CREATE JS IMAGE PRELOADER ADDED 23/08/2013 ============================/


		var shellImgLoader = new createjs.LoadQueue(true, "");
		if (shellImgLoader != null) { shellImgLoader.close();}
		shellImgLoader = new createjs.LoadQueue(true, "");
		shellImgLoader.addEventListener("fileload", handleFileLoad);
		shellImgLoader.addEventListener("progress", handleOverallProgress);
		shellImgLoader.addEventListener("fileprogress", handleFileProgress);
		shellImgLoader.addEventListener("error", handleFileError);
		shellImgLoader.setMaxConnections(5);
		function handleFileLoad(event)
			{
				//console.log(event.item.src+" : complete");
			}
        function handleFileProgress(event)
		{
			//console.log("event.item.src ="+event.item.src+" : event.progress ="+event.progress);
		}
		function handleOverallProgress(event)
		{
			var progress = shellImgLoader.progress*100;
			//console.log("progress = ",progress);
			progressTracker(progress);
			
		}
		function handleFileError(event) { /*console.log("error : "+event.item.src);*/}
		function loadAll(arr) 
		{
            while (arr.length > 0) {
               var item = arr.shift();
				shellImgLoader.loadFile(item);
            }
        }
